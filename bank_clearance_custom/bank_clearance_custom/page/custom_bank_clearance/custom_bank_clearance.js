...
class CustomBankClearance {
	...
	render() {
		this.page.main.html(`
			<style>
				...
				.bc-top{display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:26px}.bc-title{display:flex;align-items:center;gap:10px;font-size:22px;font-weight:600;letter-spacing:0}.bc-title-icon{width:22px;height:22px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;background:var(--blue);color:#fff;font-size:12px}.bc-sub{margin-top:5px;color:var(--text3);font-size:13px}.bc-actions{display:flex;gap:24px;flex-wrap:wrap;justify-content:flex-end}
				...
				.bc-table-panel{overflow:hidden;margin-top:16px}.bc-toolbar{height:61px;display:flex;align-items:center;gap:10px;padding:0 18px;border-bottom:1px solid var(--line)}.bc-search{height:34px;min-width:280px;border:1px solid var(--line);border-radius:8px;padding:0 12px;background:var(--bg);font-family:inherit}.bc-pill{height:30px;border:1px solid var(--line);border-radius:999px;background:#fff;color:var(--text2);padding:0 14px;font-size:12px;cursor:pointer;font-family:inherit}.bc-pill.on{background:var(--blue);border-color:var(--blue);color:#fff}.bc-toolbar-spacer{flex:1}.bc-sel-pill{display:none;background:var(--blue2);color:var(--blue);font-weight:500;border-radius:999px;padding:4px 10px;font-size:12px}.bc-sel-pill.show{display:inline-block}.bc-table-wrap{overflow:auto}.bc-table{width:100%;border-collapse:collapse;background:#fff}.bc-table th{height:42px;background:var(--muted);border-bottom:1px solid var(--line);color:var(--text3);font-weight:600;font-size:11px;text-transform:uppercase;letter-spacing:.04em;text-align:left;padding:0 14px;white-space:nowrap}.bc-table td{border-bottom:1px solid var(--line);padding:11px 14px;vertical-align:middle;color:var(--text)}.bc-table tr:hover{background:rgba(74,127,165,.05)}.bc-table tr.sel{background:rgba(74,127,165,.10)}.bc-check{width:16px;height:16px;border:1.5px solid var(--line2);border-radius:4px;appearance:none;-webkit-appearance:none;background:#fff;cursor:pointer;position:relative}.bc-check:checked{background:#fff;border-color:var(--blue)}.bc-check:checked:after{content:"";position:absolute;top:1px;left:4px;width:5px;height:9px;border:2px solid var(--blue);border-top:none;border-left:none;transform:rotate(42deg)}.bc-check:disabled{opacity:.45;cursor:not-allowed}.bc-doc-type{font-size:11px;color:var(--text3);font-weight:500}.bc-link{display:inline-block;color:var(--blue);font-family:inherit;font-size:12px;font-weight:500;cursor:pointer}.bc-party{font-weight:500;font-size:13px}.bc-party-sub{font-size:11px;color:var(--text3);margin-top:2px}.mono{font-family:inherit}.bc-money{display:inline-block;white-space:nowrap}.dr{color:var(--red);font-weight:500}.cr{color:var(--blue);font-weight:500}.bc-badge{display:inline-flex;align-items:center;height:24px;border-radius:999px;padding:0 9px;font-size:11px;font-weight:500}.b-dr{background:var(--red2);border:1px solid rgba(158,74,74,.24);color:var(--red)}.b-cr{background:var(--blue2);border:1px solid rgba(74,127,165,.24);color:var(--blue)}.b-ok{background:var(--green2);border:1px solid rgba(74,140,110,.24);color:var(--green)}.b-pending{background:var(--amber2);border:1px solid rgba(160,120,64,.24);color:var(--amber)}.bc-date{width:100%;min-width:132px;height:32px;box-sizing:border-box;border:1px solid var(--line);border-radius:7px;padding:0 9px;background:#fff;font-family:inherit;font-size:12px}.bc-date.filled{background:var(--green2);border-color:var(--green);color:var(--green)}.total-row td{background:var(--muted);font-weight:600}.bc-foot{min-height:54px;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:0 18px;color:var(--text3);font-size:12px}.bc-pages{display:flex;gap:4px}.bc-pages:empty{display:none}.bc-page-btn{min-width:30px;height:30px;border:1px solid var(--line);border-radius:7px;background:#fff;color:var(--text2);font-family:inherit}.bc-page-btn.on{background:var(--blue);border-color:var(--blue);color:#fff}
			...
			<div class="bc-page">
				...
				<div class="bc-panel bc-filter-panel">
					...
					<div class="bc-filter-grid">
						...
						<div class="bc-field"><label>From Date</label><input type="date" data-field="from_date" placeholder="dd/mm/yyyy"></div>
						<div class="bc-field"><label>To Date</label><input type="date" data-field="to_date" placeholder="dd/mm/yyyy"></div>
					</div>
				...
				<div class="bc-bulk" data-area="bulk">
					...
					<input type="date" data-field="bulk_date" placeholder="dd/mm/yyyy">
				...
				<div class="bc-panel bc-table-panel">
					...
					<div class="bc-foot"><span data-area="row_info">Showing 0 entries</span><div class="bc-pages" data-area="pagination"></div></div>
				</div>
			</div>
		`);
		this.render_cards({});
	...
	bind() {
		...
		root.on("click", "[data-page]", (event) => this.set_page($(event.currentTarget).data("page")));
	...
	async load_defaults() {
		...
		this.$("[data-field='from_date']").val(this.to_display_date(today.slice(0, 8) + "01"));
		this.$("[data-field='to_date']").val(this.to_display_date(today));
	...
	get_args() {
		return {
			...
			from_date: this.to_server_date(this.$("[data-field='from_date']").val()),
			to_date: this.to_server_date(this.$("[data-field='to_date']").val()),
	...
	render_cards(summary) {
		...
		const cards = [
			...
			["Bank Statement Total", this.money(summary.bank_statement_total), "Bank Transaction deposit + withdrawal", "bank", "amber"],
			...
			["Difference Amount", this.money(diff), "Bank Statement Total - Reconciled Total", "alert", "red"],
	...
	render_table() {
		...
		const page_count = Math.ceil(this.filtered.length / this.page_size) || 1;
		if (this.current_page > page_count) this.current_page = page_count;
		const start = (this.current_page - 1) * this.page_size;
		const visible_rows = this.filtered.slice(start, start + this.page_size);
		let total_deposit = 0;
		let total_withdrawal = 0;
		let cleared = 0;
		let uncleared = 0;
		const html = visible_rows.map((row) => {
			...
			return `<tr class="${selected}">
				...
				<td class="mono">${this.to_display_date(row.date || row.posting_date || "")}</td>
				...
				<td class="cr">${deposit ? this.money(deposit) : "-"}</td>
				<td class="dr">${withdrawal ? this.money(withdrawal) : "-"}</td>
				<td>${voucher_html}</td>
				<td><span class="bc-badge ${status_class}">${frappe.utils.escape_html(row.status || "")}</span></td>
				<td>${can_select ? `<input type="text" class="${date_class}" data-clearance-date data-id="${row.id}" placeholder="dd/mm/yyyy" value="${this.to_display_date(row.clearance_date || "")}">` : "-"}</td>
				<td>${action}</td>
			</tr>`;
		}).join("");
		const totals = `<tr class="total-row"><td colspan="4">TOTALS</td><td class="cr">${this.money(total_deposit)}</td><td class="dr">${this.money(total_withdrawal)}</td><td></td><td class="cr">Cleared/Reconciled: ${this.money(cleared)}</td><td class="dr">Open: ${this.money(uncleared)}</td><td></td></tr>`;
		this.$("[data-area='tbody']").html(html || `<tr><td colspan="10" style="text-align:center;color:var(--text3);padding:28px">No entries loaded</td></tr>`).append(html ? totals : "");
		const end = Math.min(start + visible_rows.length, this.filtered.length);
		this.$("[data-area='row_info']").text(this.filtered.length ? `Showing ${start + 1}-${end} of ${this.filtered.length} entries` : `Showing 0 entries`);
		this.render_pagination(page_count);
		this.update_selected_ui();
	}

	render_pagination(page_count) {
		const area = this.$("[data-area='pagination']");
		if (page_count <= 1) {
			area.empty();
			return;
		}
		...
		for (let page = 1; page <= page_count; page++) {
			buttons.push(`<button class="bc-page-btn ${page === this.current_page ? "on" : ""}" data-page="${page}">${page}</button>`);
		}
		area.html(buttons.join(""));
	}

	set_page(page) {
		this.current_page = parseInt(page, 10) || 1;
		this.render_table();
	}
	...
	set_local_clearance_date(input) {
		...
		if (!row) return;
		row.clearance_date = this.to_server_date(input.val());
	...
	async apply_bulk(all) {
		const date = this.to_server_date(this.$("[data-field='bulk_date']").val());
		if (!date) {
	...
	export_csv() {
		const rows = [["Date", "Type", "Description", "Deposit", "Withdrawal", "ERP Voucher", "Status", "Clearance Date"]];
		this.filtered.forEach((row) => rows.push([this.to_display_date(row.date || row.posting_date), row.type, row.description || row.party, row.deposit, row.withdrawal, row.erp_voucher, row.status, this.to_display_date(row.clearance_date)]));
	...
	money(value) {
		const amount = flt(value).toLocaleString("en-IN", {
			minimumFractionDigits: 2,
			maximumFractionDigits: 2,
		});
		return `<span class="bc-money">&#8377;&nbsp;${amount}</span>`;
	}
}
